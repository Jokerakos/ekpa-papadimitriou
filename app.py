...
conn = psycopg2.connect('postgres://********:********@***.***.***.**:5432/*********')
cur = conn.cursor()
...
df247 = pd.read_sql_query("select * from newscrawler where newsource = 'news247'", conn)
capital = pd.read_sql_query("select * from newscrawler where newsource = 'capital'", conn)
iefimerida = pd.read_sql_query("select * from newscrawler where newsource = 'iefimerida'", conn)
frontDf1 = vectorization(df247,capital,iefimerida)
print("frontDf1:",frontDf1.to_dict('r'))
...
def crawlTest(n, value):
    if value == "value1":
        temp = pd.read_sql_query("select * from newscrawler where DATE(time) >= CURRENT_DATE", conn)
    if value == "value2":
        temp = pd.read_sql_query("select * from newscrawler where newsource = 'news247'", conn)
    if value == "value3":
        temp = pd.read_sql_query("select * from newscrawler where newsource = 'capital'", conn)
    if value == "value4":
        temp = temp = pd.read_sql_query("select * from newscrawler where newsource = 'iefimerida'", conn)
...
def pickadate(start_date,end_date):
  query = ""
  if not start_date or not end_date:
    query = """
      select * from newscrawler where time >= now() - interval '1 days';
    """
  else:
    query=""" 
      select * from newscrawler where time between '{lower}' and '{upper}'
    ;""".format(lower=start_date,upper=end_date)
  print("query:",query)
